...
$mdFiles = [];
foreach (glob(rex_addon::get('flexshop')->getPath('docs') . '/*.md') as $file) {
...
$page = \rex_be_controller::getPageObject('flexshop/docs');

foreach ($mdFiles as $key => $mdFile) {
    $keyWithoudPrio = mb_substr($key, 3);
    $currenMDFileWithoudPrio = mb_substr($currenMDFile, 3);
    $page->addSubpage((new rex_be_page($key, rex_i18n::msg('flexshop_docs_' . $keyWithoudPrio)))
        ->setSubPath($mdFile)
        ->setHref('index.php?page=flexshop/docs&mdfile=' . $key)
        ->setIsActive($key == $currenMDFile),
    );
}

echo rex_view::title($this->i18n('flexshop'));
